...
resolve_channel_name() {
  echo -e "  ${DIM}Resolving channel info...${RESET}"

  local resolved_name=""

  # Try to resolve channel URL if it's a video/short link
  local RESOLVED_URL
  RESOLVED_URL=$(yt-dlp --skip-download --no-warnings --print "%(channel_url)s" \
    "$CHANNEL_URL" 2>/dev/null | grep -v '^$' | grep -v 'NA' | grep -v 'None' | head -1)
  if [[ -n "$RESOLVED_URL" && "$RESOLVED_URL" != "NA" && "$RESOLVED_URL" != "None" ]]; then
    CHANNEL_URL=$(normalize_channel_url "$RESOLVED_URL")
  fi

  # Try to get actual channel name via yt-dlp
  for try_url in "$CHANNEL_URL/videos" "$CHANNEL_URL" "$CHANNEL_URL/featured" "$CHANNEL_URL/playlists"; do
    resolved_name=$(yt-dlp \
      --flat-playlist \
      --playlist-items 1 \
      --print "%(uploader)s" \
      --no-warnings \
      "$try_url" 2>/dev/null | grep -v '^$' | grep -v 'NA' | grep -v 'None' | head -1)
  ...
  if [[ -n "$resolved_name" ]]; then
    ...
    CHANNEL_DIR="$DOWNLOAD_ROOT/$CHANNEL_SAFE"
    mkdir -p "$CHANNEL_DIR"
    echo -e "  ${GREEN}[OK] Channel confirmed: $CHANNEL_NAME${RESET}"
  else
    echo -e "  ${DIM}Using name from URL: $CHANNEL_NAME${RESET}"
  fi
  echo ""
}
...
confirm_download() {
  ...
  # Only NOW (after Y) do we resolve the real channel name
  resolve_channel_name
}
...
draw_progress() {
  ...
  # \r to rewrite same line
  printf "\r  ${CYAN}[${GREEN}%s${CYAN}]${WHITE} %3d%%  ${DIM}%s  %s${RESET}  " \
    "$bar" "$pct" "$speed" "$size"
}
...
stream_with_progress() {
  ...
  while IFS= read -r line; do
    ...
    if echo "$line" | grep -qE '^\[download\] Destination:'; then
      ...
      echo ""
      echo -e "  ${MAGENTA}▶ ${WHITE}${short_title}${RESET}"
      draw_progress 0 "" ""
    ...
    # ── Already downloaded ───────────────────────────────────
      ...
      echo ""
      echo -e "  ${DIM}[skip] $short_title${RESET}"
    ...
    # ── 100% / file finished ─────────────────────────────────
      ...
      vid_done=$(( vid_done + 1 ))
      echo ""
      echo -e "  ${GREEN}✔ Done  [videos: ${vid_done}  skipped: ${vid_skip}]${RESET}"
...
run_download() {
  ...
  case "$DL_MODE" in
    "uploads")
      echo -e "  ${CYAN}◆ Uploads${RESET}\n"
      local upload_dir="$CHANNEL_DIR/Uploads"
      mkdir -p "$upload_dir"
      local args_str
      args_str=$(build_ytdlp_args "$CHANNEL_URL/videos" "$upload_dir")
      yt-dlp $args_str 2>&1 | stream_with_progress "$LOG"
      ;;

    "playlists")
      echo -e "  ${CYAN}◆ Playlists${RESET}\n"
    ...
    "all"|"all_uncategorized")
      ...
      if [ "$DL_MODE" = "all_uncategorized" ]; then
        echo ""
        echo -e "  ${CYAN}◆ Uncategorized${RESET}\n"
    ...
    "latest")
      echo -e "  ${CYAN}◆ Latest videos${RESET}\n"
...
download_playlists() {
  ...
  echo -e "  ${DIM}Fetching playlist list...${RESET}"
  ...
  while IFS=$'\t' read -r pl_id pl_title; do
    ...
    echo -e "\n  ${MAGENTA}[Playlist $count] ${WHITE}$pl_title${RESET}"
